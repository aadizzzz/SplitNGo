...
const Auth = () => {
  ...
  if (user && role) {
    if (role === 'super_admin') {
      ...
      return <Navigate to="/admin/zone" replace />;
    } else if (role === 'division_admin') {
      return <Navigate to="/admin/division" replace />;
    } else if (role === 'station_admin') {
      return <Navigate to="/admin/station" replace />;
    } else if (role === 'tte') {
      return <Navigate to="/admin/tte" replace />;
    } else {
            ...
            <div className="glass-auth p-6 sm:p-8 space-y-4 sm:space-y-6">
                    ...
                    <Card className="bg-primary/5 border-primary/20">
                      ...
                      <CardContent className="space-y-3 text-xs">
                        <div className="space-y-2 p-3 bg-muted/50 rounded-lg">
                          <p className="font-semibold text-foreground">Step 1: Create First Admin Account</p>
                          <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
                            <li>Sign up normally with any email</li>
                            <li>Go to Supabase SQL Editor</li>
                            <li>Run: <code className="bg-background px-1 rounded text-primary">
                              INSERT INTO user_roles (user_id, role) VALUES ('your-user-id', 'super_admin')
                            </code></li>
                          </ol>
                        </div>
                        <div className="space-y-1">
                          <p className="font-semibold text-foreground">Step 2: Login & Manage Roles</p>
                          <p className="text-muted-foreground">
                            After creating your super admin account, you can use the Admin Dashboard to assign roles to other users.
                          </p>
                        </div>
                        <div className="space-y-1 p-2 bg-amber-500/10 border border-amber-500/20 rounded">
                          <p className="text-xs text-amber-600 dark:text-amber-400">
                            💡 <strong>Quick Tip:</strong> Create sample zones, divisions, and stations in the Super Admin dashboard first!
                          </p>
                        </div>
                      </CardContent>
                    </Card>
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-background/80 px-2 text-muted-foreground italic backdrop-blur-sm">Or continue with</span>
                </div>
              </div>
              ...
              <Button 
                type="button" 
                variant="outline" 
                className="w-full h-12 border-muted-foreground/20 hover:bg-muted/50 hover:border-primary/50 transition-all duration-300 group relative overflow-hidden"
                onClick={async () => {
                  setLoading(true);
