-- Phase 1: Admin Hierarchy System - Database Schema

-- 1. Create app_role enum (extends existing if any)
...
CREATE TABLE public.division_admins (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  division_id UUID NOT NULL REFERENCES public.divisions(id) ON DELETE CASCADE,
  assigned_by UUID NOT NULL REFERENCES auth.users(id),
...
-- 16. RLS Policies for railway_stations
...
-- 17. RLS Policies for zone_admins
...
-- 18. RLS Policies for division_admins
...
-- 19. RLS Policies for station_admins
...
-- 21. RLS Policies for audit_logs
...
CREATE INDEX idx_divisions_zone_id ON public.divisions(zone_id);
CREATE INDEX idx_railway_stations_division_id ON public.railway_stations(division_id);
CREATE INDEX idx_zone_admins_user_id ON public.zone_admins(user_id);
CREATE INDEX idx_zone_admins_zone_id ON public.zone_admins(zone_id);
CREATE INDEX idx_division_admins_user_id ON public.division_admins(user_id);
CREATE INDEX idx_division_admins_division_id ON public.division_admins(division_id);
CREATE INDEX idx_station_admins_user_id ON public.station_admins(user_id);
CREATE INDEX idx_station_admins_station_id ON public.station_admins(station_id);
CREATE INDEX idx_ttes_user_id ON public.ttes(user_id);
CREATE INDEX idx_ttes_station_id ON public.ttes(station_id);
